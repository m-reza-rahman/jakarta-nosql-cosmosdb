...
public class AzureGameStoreTest {
	...
	public void test_1_Persist() {
		...
		System.out.println("Saved orders: " + template.count(Order.class) + "\n");
	}
	...
	public void test_2_Find() {
		...
		System.out.println("Found order: " + order.get() + " by its ID: " + orderId + "\n");
	}
	...
	public void test_3_Query() {
		...
		System.out.println("Found orders with cost > 1,000: " + template.select(query).count() + "\n");

		query = select().from("Order").where("description").like("^Pinball").build();
		assertEquals(1, template.select(query).count());		
		System.out.println("Found orders for pinball: " + template.select(query).count() + "\n");

		query = select().from("Order").where("orderLines.description").eq("machine").build();
		assertEquals(2, template.select(query).count());		
		System.out.println("Found orders with a machine: " + template.select(query).count() + "\n");
	}
	...
	public void test_4_Update() {
		...
		System.out.println("Updated order: " + order + " with ID: " + orderId + "\n");		
	}
